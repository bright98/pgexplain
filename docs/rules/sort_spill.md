...
- **[HighTempBlockIO](high_temp_block_io.md)** — the broader sibling rule. Fires on any non-Sort, non-Hash node with high temp block I/O (e.g. HashAggregate, WindowAgg). If both fire on the same query, the query has multiple operations exceeding `work_mem`.
